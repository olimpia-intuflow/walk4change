...
import { api, type CommunityWalk, type LeaderboardRow } from '../lib/api'
...
export function Community() {
  ...
  return (
    <div>
      <ScreenHeader title="Społeczność" emoji="🤝" subtitle="Nikt nie musi chodzić sam. Dołącz albo zaproś kogoś." />
      ...
      <div className="space-y-3 px-5 pt-2">
        ...
        <div className="pt-3">
          <h2 className="mb-3 flex items-center gap-2 font-display text-lg font-bold text-ink">
            <Trophy size={18} className="text-sand" /> Ranking tygodnia
          </h2>
          <Card className="divide-y divide-[rgba(20,52,58,0.06)] p-2">
            {board.map((r) => (
              <div
                key={r.rank}
                className={`flex items-center gap-3 rounded-2xl px-3 py-2.5 ${r.isMe ? 'bg-sea/8' : ''}`}
              >
